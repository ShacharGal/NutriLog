...
- **Short answers** unless I ask for detail. No long explanations.
...
- When you need info from a spec file, **read only the relevant section** — don't load the whole file if you only need the DB schema. also don't print long texts into main.
...
- **Always run `npm run build` before committing.** If it fails, fix it before pushing.
...
### Git Identity
All commits MUST use this author:
```bash
...
### After Pushing
Always tell me:
1. That it's pushed and Vercel
2. If any Supabase edge functions were modified, give me the exact deploy command(s):
   ```
   npx supabase functions deploy FUNCTION_NAME --project-ref <PROJECT_REF>
   ```
3. How to verify the deploy: hard-refresh the app, open console, and what to look for.

---

## Build Version Marker
The app should have a version marker (e.g., in `src/main.tsx`) that logs to console on load. Increment the build number in every commit so I can verify deployment by checking the console.

---

## Debugging
- Add `console.log` debugging to new features, prefixed with a tag like `[LogTab]` or `[APILog]` so they're easy to filter in DevTools.
- When I report a bug, check console output first before diving into code.
...
1. Read `CLAUDE.md` (this file) — this is the only file the main agent reads directly.
2. **Use a subagent** to gather current state:
   ```
   ...
   ```
3. Resume work based on the subagent's summary — don't ask "where did we leave off?"
...
After completing a TODO step (feature, component, API route, etc.), **stop and hand off to me for testing** before moving to the next step. Don't just say "done, moving on."

**Your QA handoff message should include:**
1. **What was built** — one-liner (e.g., "LogTab chat UI with AI parsing via /api/log")
2. **How to test it** — concrete steps I can follow as a non-technical user:
   - What to run (e.g., `npm run dev`, or "open localhost:5173")
   - What to try (e.g., "type 'ate 2 eggs and toast' in the chat box and hit send")
   - What I should see if it works (e.g., "a card appears showing ~280 cal, 18g protein")
3. **Known limitations** — anything that's expected to not work yet (e.g., "graphs tab is still empty — that's step 9")

**Keep it short.** This isn't a report — it's a checklist I can walk through in 2 minutes.

**After I confirm it works** (or report issues), proceed to the next step. If I report a bug, fix it before moving on.
